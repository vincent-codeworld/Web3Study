...
func IocHandler(taker *dto.Order, obFunc func(side dto.Side) *orderbook.OrderBook) ([]*dto.OrderResult, error) {
	...
	//taker 剩余的取消，不加入order book，
	canceledState := taker.State + 2
	if canceledState == dto.OrderState_ORDER_STATE_PARTIAL_CANCELED || canceledState == dto.OrderState_ORDER_STATE_CANCELED {
		taker.State = canceledState
		result = append(result, &dto.OrderResult{
			OrderId:      taker.GetOrderId(),
			UserId:       taker.GetUserId(),
			Role:         constant.RoleTaker,
			Stp:          taker.Stp,
			CancelReason: constant.CancelReasonIOC,
		})
	}
	return result, nil
